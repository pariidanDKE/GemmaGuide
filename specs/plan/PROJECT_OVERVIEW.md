...
Gemma 4 4B-IT  ◄──── Camera image (live or uploaded)
    ...
    │       Gemma checks if the object the user asked about is in there
    │
    ├── Tool: call_dpt_head(image)
    │       Calls TIPSv2 b14-dpt
    │       Returns depth map + segmentation map
    │       Post-processes: per-class mean depth + spatial position (left/center/right)
    │       Returns structured dict: {"chair": {"distance": 1.2, "position": "left"}, ...}
    │
    └── Tool: call_encoder_zero_shot(image, class_list)
            ...
            Returns coarse segmentation mask + estimated depth for those regions
    │
    ▼
Gemma 4 reasons over:
    - Original image
    - Structured spatial data (object → distance → position)
...
**Why structured data to Gemma, not raw heatmap images?**  
More robust. Code interprets the depth map, Gemma gets a clean structured input. This avoids relying on Gemma to correctly interpret color-encoded depth values, which is an untested assumption.
