...
- [ ] T019 [P] Empirically compare depth-map colormaps in `pipeline/tips_runner.py` — test plasma, viridis, grayscale on 3 diverse images for demo display quality; set confirmed winner as `DEFAULT_COLORMAP` constant (resolves research.md Decision 3 open item)
...
- [ ] T032 [P] Add bounding-box debug utilities in `pipeline/` or `tests/` inspired by `notebooks/Gemma4_(E2B)-Multimodal.ipynb` object-detection section — include robust JSON extraction for model box outputs and optional Pascal VOC box drawing on sample images for local validation (debug only; not required in end-user UI)
